...
def create_time_aware_features(df, historical_stats=None):
    ...
    Adds:
      ...
      6) lsoa_historical_mean, lsoa_historical_std, lsoa_historical_count
         (expanding + shift(1) OR from provided historical_stats)
      7) imd_lag1 = (first rank_score) * lag_1 ; imd_historical = (first rank_score) * lsoa_historical_mean
    ...
    # 6) Historical (expanding) stats - FIXED: Use provided stats or compute expanding with shift
    if historical_stats is not None:
        # Use externally provided historical statistics (no leakage)
        df = df.merge(historical_stats, on="LSOA code", how="left")
    else:
        # Compute expanding statistics with shift(1) to avoid leakage
    ...
    # 7) IMD interactions (use first rank_score column if any)
...
def objective(trial, df_full):
    ...
    Ensures that for each fold, training features are built only on the training
    partition, and validation features are built using only historical stats from
    that training partition (no peeking at validation rows).
    """
    # 1) Build xgboost parameters from the trial
    params = {
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(date_indices)):
        ...
        # 1) Slice raw train / raw validation
        df_train_raw = df_full[df_full["Date"].isin(train_dates)].copy()
        df_val_raw   = df_full[df_full["Date"].isin(val_dates)].copy()

        # 2) Build features on train-only
        df_train_fe = create_time_aware_features(df_train_raw)

        # 3) Extract historical stats from train
        hist_stats = (
            df_train_fe.groupby("LSOA code")["Count"]
                        .agg(["mean", "std", "count"])
                        .reset_index()
        )
        hist_stats.columns = [
        ...
        # 4) Build features on validation, passing train-only historical stats
        df_val_fe = create_time_aware_features(df_val_raw, historical_stats=hist_stats)

        # 5) Identify feature columns (train_fe columns minus LSOA code, Date, Count)
        feature_cols = [c for c in df_train_fe.columns if c not in ["LSOA code", "Date", "Count"]]

        # 6) Prepare X / y
        X_tr = df_train_fe[feature_cols]
        y_tr = df_train_fe["Count"]
        X_vl = df_val_fe[feature_cols]
        y_vl = df_val_fe["Count"]

        # 7) Scale
        X_tr_s, X_vl_s, _ = time_aware_preprocess(X_tr, y_tr, X_vl, y_vl)

        # 8) Train XGBoost on this fold
...
def train_and_save():
    ...
    Each uses TimeSeriesSplit/Optuna tuning as before.
    Saves:
    ...
    # Feature engineering on full accom DataFrame (for final model)
    # FIXED: Use None for historical_stats to compute expanding stats properly
    df_full_accom = create_time_aware_features(df_accom, historical_stats=None)
    feature_cols_accom = [
    ...
    # Feature engineering on full hours DataFrame (for final model)
    # FIXED: Use None for historical_stats to compute expanding stats properly
    df_full_hours = create_time_aware_features(df_hours, historical_stats=None)
    feature_cols_hours = [
...
def cross_val_metrics(df_full):
    ...
    Ensures no leakage: train features are built on train-only, val features on train-only stats.
    """
    dates = np.sort(df_full["Date"].unique())
    tscv = TimeSeriesSplit(n_splits=N_SPLITS)
    date_indices = np.arange(len(dates))
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(date_indices)):
        ...
        # 4) Build features on val using only train stats
        df_val_fe = create_time_aware_features(df_val_raw, historical_stats=hist_stats)
        ...
        y_train = df_train_fe["Count"]
        X_val   = df_val_fe[feature_cols]
        y_val   = df_val_fe["Count"]
        ...
        # 8) Fixed hyperparameters for evaluation
        params = {
            "max_depth":        6,
            "eta":              0.1,
            "subsample":        0.8,
...
def plot_cv_performance_accom():
    ...
    # 1) Slice raw train/val
    df_train_raw = df[df["Date"].isin(train_dates)].copy()
    df_val_raw   = df[df["Date"].isin(val_dates)].copy()

    # 2) Build features on train-only - FIXED: Use None for proper expanding stats
    df_train_fe = create_time_aware_features(df_train_raw, historical_stats=None)

    # 3) Historical stats from final training data (not expanding)
    hist_stats = (
        df_train_fe.groupby("LSOA code")["Count"]
                   .agg(["mean", "std", "count"])
                   .reset_index()
    ...
    # 4) Build features on val using train-only historical stats
    df_val_fe = create_time_aware_features(df_val_raw, historical_stats=hist_stats)

    feature_cols = [c for c in df_train_fe.columns if c not in ["LSOA code", "Date", "Count"]]

    X_train, y_train = df_train_fe[feature_cols], df_train_fe["Count"]
    X_val,   y_val   = df_val_fe[feature_cols],   df_val_fe["Count"]
    ...
    df_train_fe["Pred"] = bst.predict(dtr)
    df_val_fe["Pred"]   = bst.predict(dval)
    df_train_fe["Pred"] = np.maximum(0, df_train_fe["Pred"])
    df_val_fe["Pred"]   = np.maximum(0, df_val_fe["Pred"])

    train_agg = (
        df_train_fe.groupby("Date")
                   .agg(Actual=("Count", "sum"), Predicted=("Pred", "sum"))
                   .sort_index()
    )
    val_agg = (
        df_val_fe.groupby("Date")
                 .agg(Actual=("Count", "sum"), Predicted=("Pred", "sum"))
                 .sort_index()
    )
